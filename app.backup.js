...
function selectCategory(categoryKey) {
  var validCategories = ['tools', 'cars', 'dishes', 'clothes'];
...
function updateNavLabels(category) {
  var labels = {
    tools:   { nav: 'Asboblar',       title: 'Asboblar katalogi',       sub: 'Ijaraga beriladigan asboblar ro\'yxati',       icon: '⚙',  addLabel: "+ Asbob qo'shish",      addFn: 'openToolModal()',    itemCol: 'Asboblar',      searchPh: 'Mijoz yoki asbob nomi...' },
    cars:    { nav: 'Avtomobillar',   title: 'Avtomobillar katalogi',   sub: 'Ijaraga beriladigan avtomobillar ro\'yxati',   icon: '🚗', addLabel: "+ Avtomobil qo'shish",   addFn: 'openCarModal()',     itemCol: 'Avtomobil',     searchPh: 'Mijoz yoki avtomobil nomi...' },
    dishes:  { nav: 'Idish-tovoqlar', title: 'Idish-tovoqlar katalogi', sub: 'Ijaraga beriladigan idish-tovoqlar ro\'yxati', icon: '🍽', addLabel: "+ Idish qo'shish",       addFn: 'openDishModal()',    itemCol: 'Idish-tovoq',   searchPh: 'Mijoz yoki idish nomi...' },
    clothes: { nav: 'Kiyimlar',       title: 'Kiyimlar katalogi',       sub: 'Ijaraga beriladigan kiyimlar ro\'yxati',       icon: '👔', addLabel: "+ Kiyim qo'shish",       addFn: 'openClothesModal()', itemCol: 'Kiyim',         searchPh: 'Mijoz yoki kiyim nomi...' }
  };
  var info = labels[category] || labels['tools'];
...
function initApp() {
  ...
  // Har doim items ni ham yukla (tools kategoriyasida ham kerak bo'lishi mumkin)
  ...
  if (currentCategory === 'tools') {
    ...
    if (tools.length === 0) {
      tools = [
        { id:1, name:'Beton aralashtirgich', dayRate:80000, qty:2 },
        { id:2, name:'Perforator',           dayRate:50000, qty:3 },
        { id:3, name:'Lestnitsa (3m)',        dayRate:25000, qty:4 },
        { id:4, name:'Bolgar',               dayRate:40000, qty:2 },
        { id:5, name:'Mishalka',             dayRate:25000, qty:3 },
        { id:6, name:'Apalubka (1 dona)',    dayRate:15000, qty:10 },
        { id:7, name:'Lesa (1 sekciya)',     dayRate:10000, qty:20 }
      ];
      saveTools();
    }
  }
...
function saveItems()   { try { localStorage.setItem(DB_PREFIX + 'items',   JSON.stringify(items)); } catch(e) { showToast && showToast("Xotira to'lib qoldi, eski ma'lumotlarni o'chiring", 'error'); } }
...
function buildToolPicker(selectedItems) {
  ...
  tools.forEach(function(t) {
    var sel     = selectedItems ? selectedItems.find(function(i) { return i.toolId === t.id; }) : null;
...
function onPickerChange() {
  ...
  } else if (currentCategory === 'dishes') {
...
function openAddModal(id) {
  ...
  var today = localDateTimeStr(); // local vaqt
  ...
  if (items.length > 0) {
    ...
    items.forEach(function(item) {
      if (item.toolId !== undefined) {
        ...
        var tool = tools.find(function(t) { return t.id === item.toolId; });
        if (!tool) return;
        var inUse = getToolInUseQty(item.toolId, editId || null);
        ...
        // Cars/dishes/clothes kategoriyasi
        var catItem = window.items.find(function(c) { return c.id === item.itemId; });
        if (!catItem) return;
        var inUse2 = getItemInUseQty(item.itemId, editId || null);
...
function getToolInUseQty(toolId, excludeRentalId) {
  ...
  rentals.forEach(function(r) {
    ...
    var activeItems = getActiveItems(r);
    var item = activeItems.find(function(i) { return i.toolId === toolId; });
...
function buildReturnPicker(r) {
  var activeItems = getActiveItems(r);
  var html = '<div class="tool-picker-head">&#10003; Belgilangan = qaytarilayotgan element</div>';
...
function deleteTool(id) {
  if (rentals.some(function(r) { return r.status==='active' && r.items && r.items.some(function(i){return i.toolId===id;}); })) {
...
function renderDashboard() {
  ...
  var catItemLabels = {
    tools:   { label: 'Jami asboblar',    count: tools.length },
    cars:    { label: 'Jami avtomobillar', count: items.length },
    dishes:  { label: 'Jami idishlar',     count: items.length },
    clothes: { label: 'Jami kiyimlar',     count: items.length }
  };
  var catStat = catItemLabels[currentCategory] || catItemLabels['tools'];
  var statLabel = document.getElementById('stat-tools-label');
  if (statLabel) statLabel.textContent = catStat.label;
  document.getElementById('stat-tools').textContent = catStat.count;
...
function openCheck(id) {
  ...
  var catLabels = {
    tools:   { col: 'Asbob',      icon: '⚙️',  title: 'Qurilish Asboblari' },
    cars:    { col: 'Avtomobil',  icon: '🚗',  title: 'Avtomobil Ijarasi'  },
    dishes:  { col: 'Idish',      icon: '🍽️', title: 'Idish-Tovoqlar'     },
    clothes: { col: 'Kiyim',      icon: '🤵',  title: 'Kiyim Ijarasi'      }
  };
  var cat = catLabels[currentCategory] || catLabels['tools'];
...
function renderCatalogPage() {
  ...
  else if (currentCategory === 'dishes') renderDishes();
...
function getItemInUseQty(itemId, excludeRentalId) {
  ...
  rentals.forEach(function(r) {
    ...
    if (excludeRentalId && r.id === excludeRentalId) return;
    var activeItems = getActiveItems(r);
    var found = activeItems.find(function(i) { return (i.itemId || i.toolId) === itemId; });
...
function openDishModal(id) {
  editItemId = id || null;
  dishPendingImages = [];
  document.getElementById('dish-modal-title').textContent = id ? 'Idishni tahrirlash' : "Idish qo'shish";
  ...
  if (id) {
    ...
    document.getElementById('dish-type').value = dish.type    || 'Lagan';
    ...
    document.getElementById('dish-type').value = 'Lagan';
...
function renderDishes() {
  ...
  if (!items.length) {
    grid.innerHTML    = '<div class="empty" style="grid-column:1/-1"><div class="empty-icon">&#127859;</div>Idish-tovoq qo\'shilmagan</div>';
    cardsEl.innerHTML = '<div class="empty"><div class="empty-icon">&#127859;</div>Idish-tovoq qo\'shilmagan</div>';
...
function showPage(id, el) {
  ...
  if (id === 'sozlamalar') {
    setTimeout(function() {
      ...
      if (catEl) {
        var catNames = { tools:'Qurilish asboblari', cars:'Avtomobil / RentCar', dishes:'Idish-tovoqlar', clothes:'Kiyimlar' };
        catEl.textContent = catNames[currentCategory] || '—';
      }
    }, 0);
  }
  if (id === 'asboblar') {
